...
export default function Create() {
    ...
    const handleUpload = async () => {
        if (pdfContent.length > 0) {
            try {
                let collectionRef = collection(db, "users", user.uid, "quizSets"); // Use collection instead of doc
                let docRef = await addDoc(collectionRef, { // Use addDoc instead of setDoc
    ...
    return (
        ...
        <div className ="py-4 px-12 min-h-[100dvh] bg-black text-white flex flex-col gap-12">
            ...
            <input 
                ...
                value={quizName}    
                onChange={(e) => {setQuizName(e.target.value); console.log(e.target.value)} }
    />
                {!file ? <div 
                        ...
                        <input type='file' id='file' ref={fileInputRef} style={{display: 'none'}} onChange={() => {
                    ...
                    </div> : <div className = "w-1/2 rounded-lg border-2 border-dashed border-opacity-70 border-secondary h-64 flex flex-col items-center justify-center">
                    <LuFile className='text-6xl text-secondary mb-8'/>
                        <div className="flex gap-1 justify-center items-center">
                            <h1 className=' text-secondary text-xl center'>{file.name}</h1>
                            <IoClose onClick ={handleDelete} className="text-secondary text-2xl hover:cursor-pointer w-8 h-8"/>
                        </div>
                </div>}
                <div className=" w-1/2 flex flex-col gap-4 items-center">
                    <Button className={"w-full"} icon={<FaUpload/>} text={"Upload"} onClick={handleUpload}/>
